...
def create_bar_chart2(rows, TypeS):
    ...
    for row in rows:
        ...
        items[row.year] = items.get(row.year, 0) + row.sCaseCount
        year.add(row.year)
    ...
    chart.axes(QtCore.Qt.Horizontal)[0].setTitleText("Количество страховых случаев")

    chart.setMargins(QtCore.QMargins(50, 20, 50, 0))
    return chart